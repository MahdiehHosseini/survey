  ...
  return (
    <div className={`bg-gray-200 items-center rounded-xl px-10 py-5 flex justify-between mx-7 my-10 ${path === '/' ? 'hidden' : 'flex'}`} >
        ...
        <div>
            ...
            <Link to='surveyes-list' className={`text-custom-green text-xl mx-7  ${(!path.includes('surveyes-list') && !path.includes('survey')) && 'opacity-60'}`}>لیست نظر سنجی ها</Link>
            <Link to='/users-list' className={`text-custom-green text-xl  ${!path.includes('/user') && 'opacity-60'}`}>لیست کاربران</Link>
        </div>
    </div>
  )
}
